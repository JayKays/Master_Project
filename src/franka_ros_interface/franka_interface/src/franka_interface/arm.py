...
class ArmInterface(object):
    ...
    def move_to_joint_positions(self, positions, timeout=10.0,
        ...
        #self._ctrl_manager.set_motion_controller(curr_controller) # is commented out akhil
    ...
    def get_full_state_space(self):
        state_dict = {}
        state_dict["Fx"] = self.endpoint_effort()['force'][0]
        state_dict["Fy"] = self.endpoint_effort()['force'][1]
        state_dict["Fz"] = self.endpoint_effort()['force'][2]
        state_dict["Tx"] = self.endpoint_effort()['torque'][0]
        state_dict["Ty"] = self.endpoint_effort()['torque'][1]
        state_dict["Tz"] = self.endpoint_effort()['torque'][2]
        state_dict["x"] = self.endpoint_pose()['position'][0]
        state_dict["y"] = self.endpoint_pose()['position'][1]
        state_dict["z"] = self.endpoint_pose()['position'][2]
        state_dict["Q"] = self.endpoint_pose()['orientation']
        state_dict["Vx"] = self.endpoint_velocity()['linear'][0]
        state_dict["Vy"] = self.endpoint_velocity()['linear'][1]
        state_dict["Vz"] = self.endpoint_velocity()['linear'][2]
        state_dict["Ax"] = self.endpoint_velocity()['angular'][0]
        state_dict["Ay"] = self.endpoint_velocity()['angular'][1]
        state_dict["Az"] = self.endpoint_velocity()['angular'][2]

        #y_error = self.endpoint_pose()['position'][1] - p_x_d[1]
        #force_dot = get_derivative_of_vector(force_hist,iteration,time_per_iteration)[2]
        #force_overshoot = max(F - (Fd+0.5),0)
        #state_list = [F, delta_p_z, v_z, x_error, y_error, force_dot]
    ...
    def get_6_dim_state_space(self,p_z_init,F_offset, Fd, p_x_d, force_hist,iteration,time_per_iteration):
        ...
        delta_p_z = self.endpoint_pose()['position'][2]-p_z_init
        v_z = self.endpoint_velocity()['linear'][2]
        x_error = self.endpoint_pose()['position'][0] - p_x_d[0]
        y_error = self.endpoint_pose()['position'][1] - p_x_d[1]
        force_dot = get_derivative_of_vector(force_hist,iteration,time_per_iteration)[2]
        force_overshoot = max(F - (Fd+0.5),0)
        state_list = [F, delta_p_z, v_z, x_error, y_error, force_dot]
